...
### golang 变量的作用域
Go语言中变量的有效范围如下：
- 函数级别:变量声明在函数内部,只有在函数内部才能访问,称变量为局部变量，简短声明也只能在函数内部使用
...
7. ~~go 如何做类型转换，比如 string()，是不是强类型语言不需要进行类型转换？~~
   - 空接口可以存储任意类型的数据，因此存储在接口中的数据可以进行类型转换。（不过可以看出来就是显而易见的相同类型转换，而非从类型 1 转换为类型 2）
   - 使用 `strconv` 可以将数字类型转化为字符串类型
8. ~~如何遍历一个数组~~
...
当被调用的函数返回时，这个 goroutine 也自动结束。另外注意 Go 协程应该区分于其他语言的协程

defer 确实是一个闭包，它在声明时不会立刻去执行，而是在函数 return 之后去执行的

14. ~~golang 中是否有同步函数和异步函数的概念呢~~

golang 中 goroutine 其实就是异步的，具体可以参考 async.go 
...
16. golang 项目中一般使用什么 lint 工具
暂没看到相关的项目中使用，待补充

17. ~~golang 中的 go xx 是什么意思~~
go 关键字用来创建协程，是实现并发的关键
